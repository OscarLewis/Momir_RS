...
mod tests {
    ...
    #[test(tokio::test)]
    #[ignore]
    async fn test_room() -> Result<(), Box<dyn std::error::Error>> {
        let card = load_card("./tests/walk_in_closet_test_card.json")?;
        ...
        let print = CardPrint::new(&card_type);
        print
            .render(Some(&PathBuf::from("./renders/walk_in_closet_card.png")))
            .await?;
        Ok(())
    }
